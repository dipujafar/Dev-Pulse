...
const getAllIssuesFromDB = async (query) => {
    ...
    const issuesData = await pool.query(`SELECT * FROM issues
     ${where}
     ORDER BY created_at ${orderBy}`, queryValues);
    const issues = issuesData.rows;
    if (issues.length === 0) {
        return [];
    }
    const reportersId = [...new Set(issues.map((i) => i.reporter_id))];
    const reportersResult = await pool.query(`SELECT id, name, role FROM users WHERE id = ANY($1)`, [reportersId]);
    const reporterMap = {};
...
`, [id]);
    ...
    const userData = await pool.query(`SELECT id, name, role FROM users WHERE id=$1`, [issue.reporter_id]);
...
const updateIssueIntoDB = async (payload, id, userId) => {
